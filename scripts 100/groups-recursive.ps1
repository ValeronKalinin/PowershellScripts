...
$report = Get-UserGroupMembershipRecursive a.kondratyev -Verbose
$g=$report.MemberOf
$massiv= foreach ($m in $g){ Get-ADGroup $m.SID  -Properties mail} 
$massiv | select name,mail| where mail -ne $null | export-csv -Path C:\Users\v.kalinin\Desktop\someshit1.csv -NoTypeInformation -Encoding UTF8 -Delimiter ";"


$groups=$massiv | where mail -ne $null
foreach ($group in $groups)
{
$group.Name
Add-ADGroupMember -Identity $group.DistinguishedName -Members a.kondratyev
