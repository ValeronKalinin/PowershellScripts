...
Set iLO management module to comply internal standards of JSC TK Center.

Set-iLO.ps1 –mail konstantin.andreev@stoloto.ru –exportpath d:\iloexport –ip 10.200.205.5 –defaultpass Gt85rhJJ -tag 32000
Set-iLO.ps1 –mail konstantin.andreev@stoloto.ru –exportpath d:\iloexport –ip 10.200.205.5
...
param (
    ...
    [parameter][string]$defaultpass,
    [parameter][string]$tag
#   [parameter][string]$netbiosname
...
[string]$ilooperator = "2,3,4,6"
[string]$forest = "orglot.office"

if ($ip -ne ($([ipaddress]$ip).IPAddressToString)) {
    throw "`n`nWrong IP-address!`n`n"
}
$servername = (Find-HPiLO $ip -wa 0 -ea 0).SerialNumber.trim()
if ($servername.Length -ne 10) {
    throw "`n`nNo valid iLO found!`n`n"
}
New-Item ("$exportpath\iLO-$servername.log").replace('\\','\') -Force
if (!(Test-Path ("$exportpath\iLO-$servername.log"))) {
    throw "`n`nCannot write in export folder!"
}
...
Add-Content -Path "$exportpath\$servername.log" -Value "Title: $servername"

Disable-HPiLOCertificateAuthentication
Set-HPiLOSessionTimeOut -TimeOut 120

if ($defaultpass.length -ne 8) {
...
while (($check).STATUS_MESSAGE -ne "OK") {
    ...
    Write-Host "`n<<  Attention, wrong domain password!  >>`n`Press ENTER to try again or CTRL-C to exit" -ForegroundColor Red
    $cr = Get-Credential 
    $check = Get-HPiLODirectory -Server $ip -Credential $cr
}
...
Set-HPiLOGlobalSetting -Credential $cr -Server $ip -AlertMail Yes -AlertMailEmail 'ilo@stoloto.ru' -AlertMailSenderDomain 'stoloto.ru' `
    -AlertMailSMTPPort "25" -AlertMailSMTPServer 'mail.stoloto.ru'
Start-Sleep -Seconds 5 -Verbose
...
Start-Sleep -Seconds 5 -Verbose
Add-Content -Path "$exportpath\$servername.log" -Value "Login: iload"
...
Add-Content -Path "$exportpath\$servername.log" -Value "Password: $newpass"

Start-Sleep -Seconds 5 -Verbose
Add-Content -Path "$exportpath\$servername.log" -Value "URL: https://$servername.$forest"
Add-Content -Path "$exportpath\$servername.log" -Value "Note: Asset Tag: $tag`n`
Default password: $defaultpass`n`
Added by: $mail`n`
Date: $(Get-Date -UFormat "%Y%m%d-%H%M")"

$content = Get-Content -Path "$exportpath\$servername.log"
Write-Host "`n`nSending mail to $mail"
Send-MailMessage -To $mail -From "<ilo@stoloto.ru>" -Subject "$servername $ip" -SmtpServer mail.stoloto.ru -Body "`n`n$content`n`n"
...
$(($req).CERTIFICATE_SIGNING_REQUEST) | Out-File $exportpath\$servername.req
$reqpath = ("$exportpath\$servername.req").replace('\\', '\')
$cerpath = ("$exportpath\$servername.cer").replace('\\', '\')
